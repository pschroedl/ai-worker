...
def gradio_process_image_cli(args):
    input_image = Image.open(args.input_image).convert("RGB")
    result = gradio_process_image(
        input_image, 
        args.resolution, 
    ...
    result[1].save(args.output_image)
    print(f"Processing completed. Output image saved to {args.output_image}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process an image using the gradio_process_image function.")
    parser.add_argument("input_image", type=str, help="Path to the input image.")
    parser.add_argument("output_image", type=str, help="Path to save the output image.")
    parser.add_argument("--resolution", type=int, default=1024, help="Resolution of the output image.")
    parser.add_argument("--num_inference_steps", type=int, default=20, help="Number of inference steps.")
    parser.add_argument("--strength", type=float, default=0.2, help="Strength parameter.")
    parser.add_argument("--hdr", type=float, default=0, help="HDR effect parameter.")
    parser.add_argument("--guidance_scale", type=float, default=6, help="Guidance scale parameter.")
    parser.add_argument("--controlnet_strength", type=float, default=0.75, help="ControlNet strength parameter.")
    parser.add_argument("--scheduler_name", type=str, default="DDIM", choices=["DDIM", "DPM++ 3M SDE Karras", "DPM++ 3M Karras"], help="Scheduler name.")

    args = parser.parse_args()
    gradio_process_image_cli(args)